...
describe("play settings", () => {
  ...
  it("reads valid persisted settings", () => expect(readPlaySettings({ getItem: (key) => key === PLAY_SETTINGS_KEY ? JSON.stringify({ playMode: "practice", countdownSeconds: 5, fallbackBpm: 80, hitToleranceMs: 400, showHitsWhilePlaying: true, playFullscreen: true }) : null })).toEqual({ playMode: "practice", countdownSeconds: 5, fallbackBpm: 80, hitToleranceMs: 400, showHitsWhilePlaying: true, playFullscreen: true }));
