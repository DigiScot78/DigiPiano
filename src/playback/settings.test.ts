...
describe("play settings", () => {
  ...
  it("reads valid persisted settings", () => expect(readPlaySettings({ getItem: (key) => key === PLAY_SETTINGS_KEY ? JSON.stringify({ countdownSeconds: 5, fallbackBpm: 80, hitToleranceMs: 400, showHitsWhilePlaying: true, autoHideSidebarOnPlay: true }) : null })).toEqual({ countdownSeconds: 5, fallbackBpm: 80, hitToleranceMs: 400, showHitsWhilePlaying: true, autoHideSidebarOnPlay: true }));
