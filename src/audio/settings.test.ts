...
describe("audio settings", () => {
  ...
  it("reads valid persisted settings", () => expect(readAudioSettings({ getItem: (key) => key === AUDIO_SETTINGS_KEY ? JSON.stringify({ muted: true, volume: 32 }) : null })).toEqual({ muted: true, volume: 32 }));
