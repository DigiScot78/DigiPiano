...
describe("workspace layout settings", () => {
  ...
  it("reads valid persisted settings", () => expect(readWorkspaceLayoutSettings({ getItem: (key) => key === WORKSPACE_LAYOUT_SETTINGS_KEY ? JSON.stringify({ sidebarOpen: false, sidebarWidth: 480, sidebarTab: "debug", panelOpenScoreMargin: 80, panelDockedScoreMargin: 140 }) : null })).toEqual({ sidebarOpen: false, sidebarWidth: 480, sidebarTab: "debug", panelOpenScoreMargin: 80, panelDockedScoreMargin: 140 }));
  it("migrates the legacy margin and defaults the workspace tab", () => expect(readWorkspaceLayoutSettings({ getItem: () => JSON.stringify({ sidebarOpen: true, sidebarWidth: 320, scoreMargin: 120 }) })).toEqual({ sidebarOpen: true, sidebarWidth: 320, sidebarTab: "practice", panelOpenScoreMargin: 120, panelDockedScoreMargin: 120 }));
