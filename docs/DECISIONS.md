...
Learning actions generate session-only MusicXML and replace the current score through the same normalized loading path as imported files instead of introducing a second notation or practice model. The initial supported slice is C and D major: four RH whole-note chord measures, or a two-measure one-octave scale whose key signature, written accidentals, two-staff content, and printed fingerings are deterministic. Scale activation sets the global hand mode to Both; users then filter either staff through the ordinary score controls. Stable synthetic filenames preserve exercise identity and no transport starts automatically.
