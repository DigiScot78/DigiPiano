...
Playback auto-hide is transient state layered over the persisted sidebar preference, so stopping restores only a panel that the run itself hid. Open and docked layouts own separate persisted score margins, with the legacy margin copied to both during loading. The workspace reserves score width but spans the viewport as an opaque layer above bottom surfaces in its own column. A fixed header and persisted accessible Practice/Debug tabs give the active workspace one scrolling body and leave room for future chapters, score browsing, and progress views. Settings likewise use accessible category tabs so future controls can grow without returning to one long modal column.
