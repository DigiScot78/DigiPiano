...
The score is the dominant page surface beneath a fixed application header that displays app identity, imported score identity, and the globally available Settings control. A right workspace reserves its persisted width in the score grid while its opaque viewport-fixed surface is bounded below that header and above the screen margin, layering above the bottom instruments only within that column. Its fixed file/tab header and persisted Practice/Debug tabs leave one scroll owner in the active tab body, avoiding document-scroll coupling, height coupling, and nested scrolling when Synthesia expands. The same tab shell can later host chapters, score browsing, and progress without changing workspace geometry.
