...
A full-width piano panel is fixed to the bottom of the viewport, while height-aware page clearance keeps the end of long scores reachable above it. `src/piano/piano.ts` owns persisted preference validation, standard/custom MIDI bounds, normalized white-key geometry, and expected/held/carried state priority. `PianoPanel` consumes the already hand-filtered expected event plus combined hardware/simulation held notes. Carried completed notes resolve to a neutral pressed state before correctness checks, preserving the learning model's suppression rule. Geometry is expressed from 0 to 1 across the white-key span; pixel sizing and Auto/Fit/Scroll overflow policy remain presentation concerns.

The optional Synthesia layer reuses that normalized key geometry for subtle lanes and staff-coloured note blocks derived from the active `PlaybackPlan`. It is fixed to the real keyboard strike line, mirrors the keyboard content width and scroll offset, and extends upward over the score with pointer-transparent content; only its top resize handle captures input. The persisted presentation can be transparent or opaque and can label blocks with scientific note names. Blocks render above the piano toolbar, and a block crossing the strike line fills its key with the same hand colour until its written end; a note gate freezes this state at onset. Persisted 70/100/140px-per-second presets map onset distance and written duration to vertical pixels, so panel height changes lookahead independently of speed. Very short notes retain strike-key colour for a 140ms readable minimum without extending their written blocks. The transport exposes a precise negative countdown playhead, advancing musical elapsed time, and frozen gate time to keep falling blocks on the same clock as score and piano feedback. Auto width follows only the next approaching onset when it leaves the viewport, while Fit remains static and Scroll remains manual.
