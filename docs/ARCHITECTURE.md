...
3. `src/components/ScoreRenderer.tsx` renders the MusicXML string with OpenSheetMusicDisplay, maps parsed `ScoreEvent` objects back to OSMD graphical measure/timestamp positions where available, and draws app-owned overlays for the visible score position and interaction.
...
9. The score renderer overlays transparent event hit zones above mapped `ScoreEvent` positions so visual selection resolves to stable score ranges.
