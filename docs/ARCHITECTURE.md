...
The score is the dominant page surface. A right workspace reserves its persisted width in the score grid while its opaque full-viewport surface layers above the bottom instruments only within that column. Its fixed file/settings header and persisted Practice/Debug tabs leave one scroll owner in the active tab body, avoiding height coupling and nested scrolling when Synthesia expands. The same tab shell can later host chapters, score browsing, and progress without changing workspace geometry.

Workspace layout preferences persist the right-docked sidebar's open state and 240–640px width plus separate symmetric 8–300px paper margins for open and docked layouts. Legacy single-margin storage migrates into both values. A playback-only hidden flag leaves the saved open preference untouched: an opted-in fresh run hides an open panel until the transport returns to idle, while manual restoration cancels that run's transient hide. The sticky dock's height follows the measured top of the complete fixed piano/Synthesia/timeline surface and its content owns an internal scroll region. The active margin narrows the centred score frame rather than padding notation, exposing the app background and causing OSMD to reflow systems. A `ResizeObserver` watches OSMD's actual target width, while window resize coverage handles viewport, zoom/display, and device-pixel changes; resize work is throttled, rerenders the existing OSMD instance, and rebuilds graphical anchors without reparsing or reloading the score.

A full-width piano panel is fixed to the bottom of the viewport, while height-aware page clearance keeps the end of long scores reachable above it. `src/piano/piano.ts` owns persisted preference validation, standard/custom MIDI bounds, normalized white-key geometry, and expected/held/carried state priority. `PianoPanel` consumes the already hand-filtered expected event plus combined hardware/simulation held notes. Clicking a visible key auditions it through a panel-owned synth engine that is isolated from scheduled score audio, so pointer notes can sound at any transport phase without cancelling or rescheduling the score. It follows the shared mute/volume state; raw MIDI remains silent. Carried completed notes resolve to a neutral pressed state before correctness checks, preserving the learning model's suppression rule. Geometry is expressed from 0 to 1 across the white-key span; pixel sizing and Auto/Fit/Scroll overflow policy remain presentation concerns.

The optional Synthesia layer reuses that normalized key geometry for subtle lanes and staff-coloured note blocks derived from the active `PlaybackPlan`. It is fixed to the real keyboard strike line, mirrors the keyboard content width and scroll offset, and extends upward over the score with pointer-transparent content; only its top resize handle captures input. The persisted presentation can be transparent or opaque and can label blocks with scientific note names. Blocks render above the piano toolbar, and a block crossing the strike line fills its key with the same hand colour until its written end; a note gate or manual pause freezes this state. Persisted 70/100/140px-per-second presets map onset distance and written duration to vertical pixels, so panel height changes lookahead independently of speed. Very short notes retain strike-key colour for a 140ms readable minimum without extending their written blocks. A timeline below the keyboard maps the same full-score or selection plan to a normalized playhead and event marks, snapping seek requests to playable event onsets. Auto width follows only the next approaching onset when it leaves the viewport, while Fit remains static and Scroll remains manual.
...
Manual pause adds a frozen transport phase rather than stopping the run. Fresh Play, resume, and timed seeking establish an elapsed-time origin at the displayed playable event and run the configured countdown toward it; audio scheduling rejects notes before that origin. A suspended note gate retains its satisfied-note state but ignores MIDI until the resume countdown completes. Reset is the explicit destructive transport action: it stops sound, clears results, and returns timed and untimed state to the first playable event in the active plan. The renderer groups event positions into notation systems and scrolls only when the displayed event enters a different system, using scroll margin to retain the floating toolbar and additional breathing room above the notation.
...
Pause-at-each-note mode groups hand-filtered events with the same onset into a gate of unique MIDI pitches. The transport detects crossed onsets even after a delayed animation frame, freezes musical elapsed time at the earliest gate, and requires fresh note-ons rather than accepting keys that were already held. Gate progress is intersected with the live held-note set, so releasing any partial chord member removes it and every required pitch must be down concurrently before playback rebases around the frozen onset. Pauses therefore extend wall-clock duration without compressing later timing. Between gates, the piano previews only the next unopened gate rather than duration-active score notes. The option can be changed live: enabling applies to the next future onset, while disabling an active gate resumes immediately. Gated input bypasses recorded performance results and missed-note generation; the renderer instead derives temporary green/red feedback directly from currently held notes and clears it on release.
