...
Deliverables:
- [x] Deploy the current client to Vercel. `https://digipiano.vercel.app/` is live with production-safe headers, the whole-app recovery boundary, explicit environment configuration, and user-confirmed real MIDI/audio operation.
...
**Status:** Exit gate met. Operational error reporting remains intentionally disabled until a vendor, data scope, retention policy, and consent basis are chosen.
