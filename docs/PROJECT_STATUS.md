...
- A permanent full-width practice toolbar is fixed to the bottom of the viewport so transport and view controls remain available while long scores scroll. Its left group independently toggles Piano and Synthesia with attached option popovers, its centred group owns Play/Pause, Stop, Loop, Pause-at-each-note, Clear, and Reset, and Score Audio sits at the right. Stop freezes the exact timeline position and preserves results for a fresh-countdown resume; Reset remains the explicit return to the active plan start. The optionally visible piano supports 88/76/61/49-key and validated custom ranges, three heights, Auto/Fit/Scroll width modes, optional scientific note labels, and accessible expected/correct/wrong/carried-note states. Existing collapsed-piano preferences migrate to the new piano-visibility setting.
...
- The two `ScoreRenderer` geometry failures were resolved by anchoring completed feedback to the written-note position and mirroring pointer interaction mode in a ref so batched pointer events retain the full drag preview. With permanent-toolbar/piano migration, event-backed hand-aware Play/practice expectations, tempo-aware gate previews, exact-position Stop/resume, held-note carry suppression, header-aware Synthesia bounds, score-metadata inference, arpeggio parsing/recognition/audio, rapid MIDI chord capture, clef-aware marker placement, workspace persistence/migration, playback-fullscreen ownership, responsive-renderer, click-navigation, transport, timeline, piano-audition, and authoritative pause-gate scheduling coverage, the automated suite now reports 151/151 passing tests.
