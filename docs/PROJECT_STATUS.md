...
- Timed Play can now produce a lightweight piano-like Web Audio voice from the same hand-filtered playback plan used by the score and Synthesia view. Both transport toolbars expose synchronized persisted mute/volume popovers. Audio follows score tempo, selections, hand filters, countdowns, stops, and loops; raw MIDI input remains silent.
...
- Piano pointer interaction, MIDI-input sound generation, performance export, and note-off/held-duration assessment remain deferred. The first falling-note lanes are visual and use written onset/duration only. The current score voice is synthesized; a sampled piano remains a later audio-quality upgrade.
...
- The automated suite currently reports 102/104 passing tests. Two `ScoreRenderer` geometry assertions remain failing: completed-feedback x-ordering and drag-preview width. These failures pre-date the Synthesia/audio refinements and should be investigated separately rather than hidden by loosening assertions.
- The current managed Codex environment blocks Vitest's normal config bundling and fork workers with `spawn EPERM`; `--configLoader native --pool=vmThreads --maxWorkers=1` runs the suite successfully. No in-app browser target was attached during the latest refinement, so Synthesia and score-audio behavior still need local Chromium and real-speaker verification.
