...
- Untimed scored Practice reuses the same gate, chord, arpeggio, selection, hand-filter, Stop/Reset, and performance-result machinery, but moves directly to the next gate as soon as the current expectation is satisfied. It has no inter-note tempo deadline, records wrong pitches in red and accepted pitches in green at each written location, and follows the same deferred/live marker preference as Play.
