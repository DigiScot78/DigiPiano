...
- The permanent bottom toolbar groups Piano/Synthesia view controls and compact attached chevrons at left, Play/Pause/Resume with a matching attached three-choice Play-mode modal, Stop, Loop, Clear, and Reset centrally, and Score Audio at right. The mode modal also owns the persisted live-progress marker toggle; changing mode at any transport phase resets the attempt before applying the new mode.
...
- Natural completion in every Play mode produces a balanced note score and session-only exercise history. A count-up badge beside the app name exposes last, best, average, hit/miss/bad-note breakdowns, context, and timing statistics through an accessible detail panel; persistence and richer celebratory presentation remain future work.
